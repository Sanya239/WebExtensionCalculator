...
import {type CalculateResponse, calculatorApi, type HistoryEntry,} from './api/calculatorApi.ts'
import {consumeCalculationOnOpen, readFormState, writeFormState,} from './storage/extensionStorage.ts'
...
function App() {
    ...
    const calculateExpression = useCallback(async (submittedExpression: string) => {
    ...
    useEffect(() => {
        ...
        readFormState()
            .then(async (state) => {
                ...
                const expressionToCalculate = await consumeCalculationOnOpen()
                if (isActive && expressionToCalculate) {
                    await calculateExpression(expressionToCalculate)
                }
            })
            .catch(() => undefined)
            .finally(() => {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        await calculateExpression(submittedExpression)
    }
    ...
    return (
        <main className="flex min-h-dvh items-center justify-center bg-stone-100 p-4 text-stone-950">
            <section
                className="flex h-[560px] w-full max-w-[420px] flex-col rounded-[2rem] border border-stone-200 bg-white p-5 shadow-[0_16px_45px_rgba(28,25,23,0.08)]">
