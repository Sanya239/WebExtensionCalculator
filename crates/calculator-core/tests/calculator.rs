use calculator_core::{CalculationError, CalculationErrorKind, calculate};

#[test]
fn correct_precedence() {
    assert_eq!(
        calculate("2 + 3 * 4").unwrap(),
        14.0
    );
    assert_eq!(
        calculate("(2 + 3) * 4").unwrap(),
...
fn parse_error() {
    ...
    let error = error
        .downcast_ref::<CalculationError>()
        .expect("error must retain calculation details");
